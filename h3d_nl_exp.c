...
double calc_delta_E(spin_t* temp_spin, spin_t* spin, int layer, int row, int col){
  ...
  gsl_vector_set(neighbor_vector, 0, -spin->J_INTRA[0]*lattice[layer][(((row-1)%NUM_R) + NUM_R) % NUM_R][col].x
                                    - spin->J_INTRA[1]*lattice[layer][(((row+1)%NUM_R) + NUM_R) % NUM_R][col].x
                                    - spin->J_INTRA[3]*lattice[layer][row][(((col-1)%NUM_C) + NUM_C) % NUM_C].x
                                    - spin->J_INTRA[2]*lattice[layer][row][(((col+1)%NUM_C) + NUM_C) % NUM_C].x);
  gsl_vector_set(neighbor_vector, 1, -spin->J_INTRA[0]*lattice[layer][(((row-1)%NUM_R) + NUM_R) % NUM_R][col].y
                                    - spin->J_INTRA[1]*lattice[layer][(((row+1)%NUM_R) + NUM_R) % NUM_R][col].y
                                    - spin->J_INTRA[3]*lattice[layer][row][(((col-1)%NUM_C) + NUM_C) % NUM_C].y
                                    - spin->J_INTRA[2]*lattice[layer][row][(((col+1)%NUM_C) + NUM_C) % NUM_C].y);
  gsl_vector_set(neighbor_vector, 2, -spin->J_INTRA[0]*lattice[layer][(((row-1)%NUM_R) + NUM_R) % NUM_R][col].z
                                    - spin->J_INTRA[1]*lattice[layer][(((row+1)%NUM_R) + NUM_R) % NUM_R][col].z
...
int M_v_B(double** results){
	...
	if(results[sample_counter][NUM_L + 2] > high_Q){
	  ...
	  for(j = 0; j < NUM_L; j++){
	    for(k = 0; k < NUM_R; k++){
	      for(l=0; l < NUM_C; l++){
		lattice_copy[j][k][l].x = lattice[j][k][l].x;
		lattice_copy[j][k][l].y = lattice[j][k][l].y;
		lattice_copy[j][k][l].z = lattice[j][k][l].z;
	      }
	    }
	  }
	}
	*/
	...
	for(l = 0; l < NUM_C; l++){
	  fprintf(fptr, "[");
	  fprintf(fptr, "%f,%f,%f", lattice_copy[j][k][l].x, lattice_copy[j][k][l].y, lattice_copy[j][k][l].z);
	  fprintf(fptr, "]");
	  if(l < NUM_C - 1)
	    fprintf(fptr, ",");
	}
